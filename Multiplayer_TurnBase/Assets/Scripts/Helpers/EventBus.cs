...
namespace JHelpers
{
    public class EventBus
    {
        ...
        public void SubscribeImmediately<T>(Action<T> callback, int priority = 0) where T : ISignal
        {
            string key = typeof(T).Name;
            ...
            {
                _signalCallbacks.Add(key, new() { new CallbackWithPriority(priority, callback) });
            }

            _signalCallbacks[key] = _signalCallbacks[key].OrderByDescending(x => x.Priority).ToList();
        }

        public void UnsubscribeImmediately<T>(Action<T> callback) where T : ISignal
        {
            string key = typeof(T).Name;
